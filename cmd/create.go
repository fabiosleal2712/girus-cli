...
func setupPortForward(namespace string) error {
	...
	fmt.Println("   Configurando port-forward para o backend (8084)...")
	backendCmd := fmt.Sprintf("kubectl port-forward -n %s svc/girus-backend 8084:8084 --address 0.0.0.0 > /dev/null 2>&1 &", namespace)
	...
	backendOK := false
	for i := 0; i < 5; i++ {
		healthCmd := exec.Command("curl", "-s", "--max-time", "2", "http://localhost:8084/api/v1/health")
	...
	// ------------------------------------------------------------------------
	fmt.Println("   Configurando port-forward para o frontend (8002)...")
...
# Mata qualquer processo existente na porta 8002
kill $(lsof -t -i:8002) 2>/dev/null || true
sleep 1
# Inicia o port-forward
nohup kubectl port-forward -n NAMESPACE svc/girus-frontend 8002:80 --address 0.0.0.0 > /dev/null 2>&1 &
echo $!  # Retorna o PID
`
	...
	if err == nil {
		...
		for i := 0; i < 5; i++ {
			checkCmd := exec.Command("curl", "-s", "--max-time", "2", "-o", "/dev/null", "-w", "%{http_code}", "http://localhost:8002")
			var out bytes.Buffer
			checkCmd.Stdout = &out
	...
	if !frontendSuccess {
		...
		cmd := exec.Command("kubectl", "port-forward", "-n", namespace, "svc/girus-frontend", "8002:80", "--address", "0.0.0.0")
	...
	if !frontendSuccess {
		...
		finalCmd := fmt.Sprintf("kubectl port-forward -n %s deployment/girus-frontend 8002:80 --address 0.0.0.0 > /dev/null 2>&1 &", namespace)
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if !skipPortForward {
			...
			if err := setupPortForward("girus"); err != nil {
				...
				fmt.Println("kubectl port-forward -n girus svc/girus-backend 8084:8084 --address 0.0.0.0")
				fmt.Println("kubectl port-forward -n girus svc/girus-frontend 8002:80 --address 0.0.0.0")
			} else {
				fmt.Println("✅")
				fmt.Println("Acesso configurado com sucesso!")
				fmt.Println("📊 Backend: http://localhost:8084")
				fmt.Println("🖥️  Frontend: http://localhost:8002")

				// Abrir o navegador se não foi especificado para pular
				if !skipBrowser {
					fmt.Println("\n🌐 Abrindo navegador com o Girus...")
					if err := openBrowser("http://localhost:8002"); err != nil {
						fmt.Printf("⚠️  Não foi possível abrir o navegador: %v\n", err)
						fmt.Println("   Acesse manualmente: http://localhost:8002")
					}
				}
			}
		} else {
...
func addLabFromFile(labFile string, verboseMode bool) {
	...
	if portForwardStatus {
		...
		if err != nil {
			...
			fmt.Println("   🔹 Backend: http://localhost:8084")
			fmt.Println("   🔹 Frontend: http://localhost:8002")
		}
	} else {
		// Verificar conexão com o frontend mesmo que o port-forward não seja necessário
		checkCmd := exec.Command("curl", "-s", "--max-time", "1", "-o", "/dev/null", "-w", "%{http_code}", "http://localhost:8002")
		var out bytes.Buffer
		checkCmd.Stdout = &out
		...
		if checkCmd.Run() != nil || !strings.Contains(strings.TrimSpace(out.String()), "200") {
			...
			if err != nil {
				fmt.Println("   ⚠️", err)
				fmt.Println("   Configure manualmente: kubectl port-forward -n girus svc/girus-frontend 8002:80 --address 0.0.0.0")
			} else {
...
func checkPortForwardNeeded() bool {
	...
	// Verificar se a porta 8084 (backend) está em uso
	backendPortCmd := exec.Command("lsof", "-i", ":8084")
	if backendPortCmd.Run() != nil {
		// Porta 8084 não está em uso, precisamos de port-forward
		backendNeeded = true
	} else {
		...
		backendProcessCmd := exec.Command("sh", "-c", "ps -eo pid,cmd | grep 'kubectl port-forward' | grep '8084' | grep -v grep")
		if backendProcessCmd.Run() != nil {
			...
			backendHealthCmd := exec.Command("curl", "-s", "--head", "--max-time", "2", "http://localhost:8084/api/v1/health")
			backendNeeded = backendHealthCmd.Run() != nil // Retorna true (precisa de port-forward) se o comando falhar
		}
	}

	// Verificar se a porta 8002 (frontend) está em uso
	frontendPortCmd := exec.Command("lsof", "-i", ":8002")
	if frontendPortCmd.Run() != nil {
		// Porta 8002 não está em uso, precisamos de port-forward
		frontendNeeded = true
	} else {
		// Porta está em uso, mas precisamos verificar se é o kubectl port-forward e se está funcional
		// Verificar se o processo é kubectl port-forward
		frontendProcessCmd := exec.Command("sh", "-c", "ps -eo pid,cmd | grep 'kubectl port-forward' | grep '8002' | grep -v grep")
		if frontendProcessCmd.Run() != nil {
			// Não encontrou processo de port-forward ativo ou válido
			frontendNeeded = true
		} else {
			// Verificar se a conexão com o frontend está funcionando
			frontendCheckCmd := exec.Command("curl", "-s", "--max-time", "2", "-o", "/dev/null", "-w", "%{http_code}", "http://localhost:8002")
			var out bytes.Buffer
